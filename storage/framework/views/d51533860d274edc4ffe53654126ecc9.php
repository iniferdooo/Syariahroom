...
<?php $__env->startSection('title', 'Role | syariahrooms'); ?>
...
<?php $__env->startSection('content'); ?>
                ...
                <div class="col">
                    <div class="row justify-content-between my-10 gap-10 px-0">
                        <div class="row mx-md-0 col-auto mx-auto gap-10">
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('role-store')): ?>
                                <button class="btn btn-primary col col-sm-auto add-role"><i
                                        class="ri-add-line remix-icon"></i><span>Tambah Role</span></button>
                                <button class="btn btn-primary col col-sm-auto add-permission"><i
                                        class="ri-add-line remix-icon"></i><span>Tambah Permission</span></button>
                            <?php endif; ?>
                        </div>
                        <div class="mx-md-0 col-auto mx-auto">
                            <div class="input-group align-items-center">
                                ...
                                <input class="form-control border-start-0 ps-8" id="search_role" name="search_role"
                                    type="text" value="" placeholder="Search Role">
                            </div>
                        </div>
                    </div>
    ...
    <?php $__env->startSection('modal'); ?>
        ...
        <div class="modal fade" id="modalAddPermission" tabindex="-1" role="dialog"
            aria-labelledby="modalAddPermissionLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form id="formAddPermission">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modalAddPermissionLabel">Tambah Permission</h5>
                            <button class="btn-close hp-bg-none d-flex align-items-center justify-content-center"
                                data-bs-dismiss="modal" type="button" aria-label="Close">
                                <i class="ri-close-line hp-text-color-dark-0 lh-1" style="font-size: 24px;"></i>
                            </button>
                        </div>
                        <div class="modal-body body-permission">
                            <input class="permission_id" id="permission_id" name="id" type="text" hidden>
                            <div class="form-group">
                                <div class="form-group">
                                    <label for="name">Nama</label>
                                    <input class="form-control name" id="name" name="name" type="text"
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-primary btn-save-AddPermission"><i
...
<script type="text/javascript">
    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->any(['role-store', 'role-update'])): ?>
    ...
    function loadRole(page, search) {
        ...
        $.ajax({
            type: "get",
            url: "<?php echo e(route('dashboard.role.data')); ?>",
            data: {
                'search': search,
                'page': page
            },
            success: function(res) {
                ...
                if (res.data.data.length > 0) {
                    ...
                    $.each(res.data.data, (k, v) => {
                        action = `
                        <a type="button" href="<?php echo e(route('dashboard.permission.index', ':id')); ?>" class="btn btn-sm btn-primary" title="Permission"><i class="icofont icofont-verification-check"></i></a>
    ...
    $('body').on('click', '.detail-role', function() {
        var id = $(this).data('id')
        $.get("<?php echo e(route('dashboard.role.show', ':id')); ?>".replace(':id', id), function(data) {
    ...
    $('#formRole').on('submit', function(e) {
        ...
        $.ajax({
            data: $(this).serialize(),
            url: "<?php echo e(route('dashboard.role.store')); ?>",
            type: "POST",
            dataType: 'json',
    ...
    $('#formAddPermission').on('submit', function(e) {
        ...
        $.ajax({
            data: $(this).serialize(),
            url: "<?php echo e(route('dashboard.permission.addPermission')); ?>",
            type: "POST",
            dataType: 'json',
            success: function(data) {
                $(this).trigger("reset")
    ...
    $('.btn-delete-role').on('click', function() {
        ...
        $.ajax({
            type: "DELETE",
            url: "<?php echo e(route('dashboard.role.destroy', ':id')); ?>".replace(':id', id),
            success: function(data) {
                $(this).trigger("reset")
                $('#modalRole').modal('hide')
                swal("Success...", data.message, "success")
...
<?php echo $__env->make('master.layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Syariahroom\resources\views/master/dashboard/role-permission/role.blade.php ENDPATH**/ ?>